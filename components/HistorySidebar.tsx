...
const HistorySidebar = ({ history, syncStatus, username, avatar }: HistorySidebarProps) => {
  return (
    <aside className="bg-white/70 backdrop-blur-lg rounded-[2.5rem] p-5 shadow-2xl shadow-slate-200/50 flex flex-col h-full max-h-[calc(100vh-10rem)] border border-white">
      {/* Cloud Status Card */}
      <div className="mb-6 p-5 bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-[2rem] text-white shadow-xl shadow-indigo-200 relative overflow-hidden">
        <div className="relative z-10 flex items-center gap-4">
          {avatar ? (
            <img src={avatar} className="w-12 h-12 rounded-2xl border-2 border-white/20 shadow-lg" alt="User" />
          ) : (
        ...
        <div className="absolute -right-6 -bottom-6 opacity-10">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-32 w-32" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A10.003 10.003 0 0112 3v12m0 0h6m-6 0H6" /></svg>
        </div>
      </div>
        ...
        {history.length === 0 ? (
          ...
          history.map(item => (
            <div key={item.timestamp} className="history-item border border-slate-50 rounded-[1.5rem] p-4 shadow-sm hover:shadow-xl hover:scale-[1.01] transition-all bg-white/50 hover:bg-white group">
              ...
              <div className="bg-slate-50/50 rounded-2xl p-2.5 border border-slate-100 group-hover:bg-white transition-colors">
