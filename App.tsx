...
const STORAGE_PREFIX = 'chathub_v38_';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f8] font-sans selection:bg-indigo-100 relative overflow-hidden text-slate-900" dir="rtl">
      {/* Dynamic Glowing Backgrounds - Much brighter and more colorful for v38 */}
      <div className="fixed -top-[10%] -left-[5%] w-[60%] h-[60%] bg-indigo-200/50 rounded-full blur-[120px] pointer-events-none" />
      <div className="fixed -bottom-[10%] -right-[5%] w-[50%] h-[50%] bg-teal-100/50 rounded-full blur-[120px] pointer-events-none" />
      <div className="fixed top-1/4 right-1/4 w-[30%] h-[30%] bg-purple-100/40 rounded-full blur-[100px] pointer-events-none" />

      <div className="relative z-10 max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8 h-screen flex flex-col gap-6">
        
        {/* Refined Header - White Glass */}
        <header className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white/60 backdrop-blur-3xl p-4 px-8 rounded-[2.5rem] border border-white shadow-xl">
          <div className="flex items-center gap-10">
            <div className="flex items-center gap-4">
               <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center shadow-2xl shadow-indigo-200 rotate-6 hover:rotate-0 transition-all duration-500 cursor-pointer group border border-white/20">
               ...
               <div className="flex flex-col">
                  <span className="text-2xl font-black italic tracking-tighter text-slate-800 leading-none font-assistant">ChatHub</span>
                  <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest opacity-80 mt-1">גרסת פרימיום v38</span>
               </div>
            </div>

            <nav className="hidden md:flex items-center gap-1 bg-slate-200/40 p-1 rounded-2xl border border-white/50">
          ...
          <div className="flex items-center gap-5">
             <div className="hidden lg:flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-50 border border-indigo-100 text-[10px] font-black uppercase text-indigo-600 shadow-sm">
                <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse shadow-[0_0_8px_rgba(99,102,241,0.5)]" />
                גיבוי מקומי פעיל
             </div>
             {user && (
               <div className="flex items-center gap-3 p-1.5 pr-1.5 pl-5 rounded-full bg-white border border-slate-100 shadow-lg shadow-slate-200/50 group">
                  <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-black overflow-hidden border border-indigo-200 order-2">
                  ...
                  <div className="flex flex-col items-end leading-tight order-1">
                    <span className="text-sm font-bold text-slate-800 font-assistant">{user.username}</span>
                    <button onClick={handleLogout} className="text-[10px] font-black text-slate-400 hover:text-red-500 transition-colors uppercase tracking-widest mt-0.5">נתק חשבון</button>
                  </div>
               </div>
             )}
          </div>
